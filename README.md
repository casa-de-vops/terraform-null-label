...
This module generates IDs using the following convention by default: `{organization_code}-{application}-{location_code}-{environment_code}-{attributes}`.
...
- If `attributes` is excluded but `organization`, `application`, `location`, and `environment` are included, `id` will look like `{organization_code}-{application}-{location_code}-{environment_code}`.</br>
...
The **Starwinds** convention is to use labels as follows:
- `organization`: A short (3-4 letters) abbreviation of the company name, to ensure globally unique IDs for things like S3 buckets
- `application`: The name of the component that owns the resources, such as `eks` or `rds`
- `location`: A short abbreviation for the Azure region hosting the resource, or `gbl` for resources like IAM roles that have no region
...
module "eg_prod_bastion_label" {
  source   = "git::git@ssh.dev.azure.com:v3/TruistInsurance/Terraform%20Enterprise%20Modules/terraform-null-label"
  # Cloud Posse recommends pinning every module to a specific version
  # version = "x.x.x"

  organization   = "starwind"
  application    = "bastion"
  location       = "eastus2"
  environment    = "prod"
  attributes     = ["public"]
...
This will create an `id` with the value of `22-bastion-ue2-s-public` because when generating `id`, the default order is `organization_code`, `application`, `location_code`,  `environment_code`, `attributes`
...
```hcl
resource "azurerm_resource_group" "eg_prod_bastion_public" {
  name     = module.eg_prod_bastion_label.id_resource.resource_group
  location = module.eg_prod_bastion_label.location
